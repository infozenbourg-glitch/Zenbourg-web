    ...
    switch (currentStep) {
      case "name":
        ...
        await simulateTyping(() => {
          addMessage(
            `${userInput}!  please share your email address`,
            false,
          )
        })
        break
      ...
      case "email":
        ...
        await simulateTyping(() => {
          addMessage(" What's your phone number? .", false)
        })
        break
  ...
  const handleBookingChoice = async (choice: string, userData: UserData) => {
    ...
    try {
      if (choice === "Yes, book a call") {
        ...
        await simulateTyping(() => {
          addMessage(
            "Perfect! You'll be redirected to our booking page to choose a date and time. Our team will reach out within 24 hours! 📅",
            false,
          )
        })
        ...
        await simulateTyping(() => {
          addMessage(
            "Great! We'll send your detailed quote within 24 hours and follow up with a call. 📋✨",
            false,
          )
        })
      } else {
        ...
        await simulateTyping(() => {
          addMessage(
            "No problem! We've saved your info. Our team will send details soon, and you can explore our website or contact us anytime. 📚",
            false,
          )
        })
      }
    ...
    <div ref={chatContainerRef} className="fixed bottom-4 right-4 z-50">
      ...
      {isOpen && (
        <div className="fixed inset-x-4 bottom-20 top-4 sm:right-4 sm:left-auto sm:w-80 md:w-96 z-50 animate-in slide-in-from-right-5 duration-300">
          <Card className="w-full h-full shadow-2xl border-0 bg-white/95 dark:bg-blue-900/95 backdrop-blur-xl flex flex-col overflow-hidden">
            <CardHeader className="pb-3 bg-blue-00  text-white rounded-t-lg">
                ...
                {messages.map((message) => (
                  <div key={message.id} className={`flex ${message.isUser ? "justify-end" : "justify-start"}`}>
                    <div className="max-w-[85%]">
                      ...
                      <div
                        className={`p-2.5 sm:p-3 rounded-2xl text-xs sm:text-sm shadow-lg whitespace-pre-line leading-relaxed ${message.isUser
              ...
              <div className="p-3 sm:p-4 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 flex-shrink-0">
                <div className="flex space-x-2 mb-2">
                  ...
                  <Button
                    ...
                    size="sm"
                    className="bg-blue-600  hover:from-blue-700 hover:to-purple-700 rounded-full h-8 w-8 sm:h-10 sm:w-10 p-0 flex-shrink-0"
                  >
